    ...
    <div style={{background: 'linear-gradient(135deg, #f0f5ff 0%, #e8f0fe 100%)', minHeight: '100vh', padding: '24px'}}>
      ...
      <div className="purchases-container">
        <div className="purchases-header">
          <h1 style={{color: '#0052CC'}}>Sales</h1>
          <button className="btn btn-primary" onClick={() => setShowForm(true)} style={{background: '#0052CC'}}>
            + New Sale
          </button>
        </div>
          ...
          <div className="purchases-table">
            <table>
              <thead style={{backgroundColor: '#f0f5ff'}}>
                <tr>
                  <th style={{color: '#0052CC'}}>Invoice</th>
                  <th style={{color: '#0052CC'}}>Date</th>
                  <th style={{color: '#0052CC'}}>Total</th>
                  <th style={{color: '#0052CC'}}>Status</th>
                  <th style={{color: '#0052CC'}}>Actions</th>
                </tr>
              </thead>
              <tbody>
                {sales.map((sale) => (
                  <tr key={sale.id} style={{backgroundColor: 'white'}} onMouseEnter={(e) => e.target.style.backgroundColor = '#f0f5ff'} onMouseLeave={(e) => e.target.style.backgroundColor = 'white'}>
        ...
        {showForm && (
          <div className="modal-overlay" onClick={resetForm}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              <div className="modal-header" style={{backgroundColor: '#f0f5ff', borderBottom: '2px solid #0052CC'}}>
                <h2 style={{color: '#0052CC'}}>New Sale</h2>
              ...
              <form onSubmit={handleSubmit} className="purchase-form">
                ...
                <div className="items-section">
                  ...
                  {formData.items.length > 0 && (
                    <table className="items-table">
                      <thead style={{backgroundColor: '#f0f5ff'}}>
                        <tr><th style={{color: '#0052CC'}}>Material</th><th style={{color: '#0052CC'}}>Qty</th><th style={{color: '#0052CC'}}>Price</th><th style={{color: '#0052CC'}}>Total</th></tr>
                      </thead>
                      <tbody>
                ...
                <div className="modal-footer">
                  ...
                  <button type="submit" className="btn btn-primary" style={{background: '#0052CC'}}>Create Sale</button>
                </div>
              </form>
            </div>
          </div>
